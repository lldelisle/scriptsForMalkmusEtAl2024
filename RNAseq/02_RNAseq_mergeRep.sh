...
#SBATCH --chdir /scratch/ldelisle/Sheth/RNAseq/ # This directory must exist, this is where will be the error and out files
## Specific to baobab:
...
filePathForTable="${gitHubDirectory}/RNAseq/table_rep.txt"
...
for s in $samples; do
    if [ -e ${dirPathWithResults}/STAR/${s}/${s}_Uniq_norm.bw ]; then
        allBW="${allBW} ${dirPathWithResults}/STAR/${s}/${s}_Uniq_norm.bw"
    else
        echo "Uniq norm bw not found for $s"
        noGlobal=1
    fi
    if [ -e ${dirPathWithResults}/STAR/${s}/${s}_Uniq_fwd_norm.bw ]; then
        allBWfwd="${allBWfwd} ${dirPathWithResults}/STAR/${s}/${s}_Uniq_fwd_norm.bw"
    else
        doStrand=0
    fi
    if [ -e ${dirPathWithResults}/STAR/${s}/${s}_Uniq_rev_norm.bw ]; then
        allBWrev="${allBWrev} ${dirPathWithResults}/STAR/${s}/${s}_Uniq_rev_norm.bw"
    else
        doStrand=0
    fi
done
